"""
brain/prompts/preprocessing/pdf.py
-----------------------------------
Preprocesado de documentos PDF.

v3 — build_focused desde bloques del extractor:
  build_focused(blocks, full_text, max_chars) usa los bloques del PDFExtractor
  que detecta headings por tamaño de fuente y los marca con '# Título'.
  Todos los headings detectados llegan al LLM correctamente.

v2 — chunked-friendly (preprocess sobre full_text):
  Inyecta headers ## usando el TOC detectado por puntos/tabs.
  Fallback cuando no hay bloques disponibles.
"""
...
def build_focused(blocks: list[dict], full_text: str, max_chars: int) -> str:
    """
    v3 — Construye el processed_text desde los bloques del PDFExtractor.

    El PDFExtractor detecta headings por tamaño de fuente y los emite como
    '# Heading' en el contenido de cada bloque. Los callouts y código también
    tienen su content_type correcto.

    Ventaja: todos los headings reales del PDF (detectados por font size)
    llegan al LLM, no solo los del TOC explícito.
    ...
    for block in blocks:
        ...
        # Callouts y tablas van directamente
        if content_type in ("table", "callout", "footnote"):
            parts.append(content)
            continue

        # Normalizar headings: añadir un nivel # para que ## sea el separador
        # El PDFExtractor emite '# Heading' para headings detectados por font
        lines = content.split("\n")
        converted: list[str] = []
        for line in lines:
