"""
synthesizer.py  (v6 — Planner-aware)
-------------------------------------
...
CAMBIOS v6 respecto a v3:
  - Integra get_synthesis_calls() del paquete brain.prompts:
      ...
      * medium con docs heterogéneos → 2 llamadas (Summary+CK / razonamiento)
      * resto → 1 llamada
  - Soporta el placeholder {call1_result} para encadenar llamadas
  - Mantiene chunked legacy intacto para docs muy grandes (red de seguridad)
...
FLUJO:
  ...
  2. preprocesado de texto (max_chars ajustado al modelo)
  3. Decisión:
     - Doc grande > chunk_trigger → chunked legacy v3 (sin cambios)
     - Resto + V5 ON                → planner v5: 1 o N llamadas
     - V5 OFF                        → single-call legacy v3
...
class DocumentSynthesizer:
    ...
    def synthesize(
        ...
        # =====================================================================
        # PASO 2c-v5: Síntesis con planificador (1 o N llamadas según plan)
        # Si SYNTHESIS_V5_ENABLED=false → cae al flujo single-call legacy abajo.
        # Si V5 da error → fallback al flujo single-call legacy.
        # =====================================================================
