"""
synthesizer.py  (v5.0 — Plan-Based Multi-Call)
-----------------------------------------------
Motor de síntesis híbrido con planificador v5.

CAMBIOS v5.0 respecto a v3.0:
  - El planificador (brain.prompts.get_synthesis_calls) decide N llamadas LLM:
      * small  → 1 llamada (capacidad limitada)
      * medium → 2 llamadas (Summary+CK / razonamiento) si doc heterogéneo
      * large  → 1 llamada (capacidad suficiente)
      * docs muy grandes → chunked (legacy v3, sin cambios)
  - Soporta el placeholder {call1_result} para encadenar llamadas
  - Mantiene model_profiles.py (v3) para detección de tier por modelo
  - Mantiene chunked legacy intacto como red de seguridad

FLAG DE CONTROL:
  SYNTHESIS_V5_ENABLED=true  (defecto) → v5 activo
  SYNTHESIS_V5_ENABLED=false           → solo legacy/v3

FLUJO v5:
  1. passport_builder.py → frontmatter + entities + tags (SIN LLM)
  2. preprocesado de texto (max_chars adaptado al modelo)
  3. Decisión:
     - Si doc grande > chunk_trigger → chunked legacy
     - Si no → planner v5: 1 o N llamadas según perfil + contenido
  4. Merge final + validación
...
class DocumentSynthesizer:
    ...
    def synthesize(
        ...
        # max_chars: limitado por el tipo de fichero Y por el presupuesto del modelo
        type_max = _MAX_CHARS.get(ft, _DEFAULT_MAX_CHARS)
        if profile:
            max_chars = min(profile.context_chars, type_max)
        else:
            max_chars = type_max
        ...
        # =====================================================================
        # PASO 2c: Síntesis con planificador v5 (1 o N llamadas según plan)
        # Si SYNTHESIS_V5_ENABLED=false, cae a flujo single-call legacy v3.
        # =====================================================================
        if SYNTHESIS_V5_ENABLED:
            try:
                ...
                log.info(
                    "[v5] Plan para '%s' (tier=%s): %s",
                    source, prompt_tier, explain_plan(v5_plan),
                )

                # Ejecutar todas las llamadas del plan en secuencia
                v5_results: list[str] = []
                ...
                log.info(
                    "[v5] Síntesis OK para '%s': %d chars, %d tags, %d entities "
                    "(tier=%s, n_calls=%d, provider=%s)",
                    source, len(md_final), len(final_tags), len(final_entities),
                    prompt_tier, len(v5_calls), effective_provider,
                )
                return {
                    "md_content": md_final,
                    "tags": final_tags if final_tags else tags_base,
                ...
                log.warning(
                    "[v5] Síntesis falló para '%s' (%s). Fallback a single-call legacy.",
                    source, v5_exc,
                )
                # Fall-through al flujo single-call legacy

        # =====================================================================
        # PASO 2c LEGACY: Síntesis single-call (v3) — fallback de v5 o V5 OFF
        # =====================================================================
